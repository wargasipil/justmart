...
- **Restore is intentionally manual** (needs maintenance/read-only mode UX). Documented in [DEPLOYMENT.md](DEPLOYMENT.md): `gunzip < database.sql.gz | docker compose exec -T db psql -U justmart justmart` (or load the .sql directly on Windows).
