...
**Every ConnectRPC handler at `backend/internal/service/<domain>/<rpc>.go` MUST have a co-located unit test `backend/internal/service/<domain>/<rpc>_test.go` in the same directory** (e.g. `auth/login.go` → `auth/login_test.go`). **A new RPC ships with its test in the same change — no exceptions.** These complement (don't replace) the over-the-wire `e2e/` suite: they're fast and pin each handler in isolation.
- **Must pass on BOTH SQLite and Postgres (HARD RULE).** Tests MUST be engine-agnostic: go through `servicetest`, never hardcode a driver, dialect-specific SQL, a SQLite-seeded id, or assertions that depend on SQLite quirks (lax typing, `LIKE` case, `bool`-as-int). Use the `common.*` dialect helpers (`LikeOp`/`EpochExpr`/`DayKeyExpr`/`RowLock`), and resolve seeded rows at runtime (e.g. `db.Where("is_default").First(&wh)` for the MAIN warehouse) — don't paste the `00000000-…-a1` literal.
- **Run with `make test-unit`** (SQLite, fast, no live DB) and **`make test-unit-postgres`** (sets `JUSTMART_TEST_DB_DRIVER=postgres`; needs `make up` — each test gets an isolated throwaway PG **schema**, the test files are identical). **`make test-unit-all`** runs both and is the gate (what `make test-all` invokes ahead of `test-e2e`). SQLite is self-contained (no `JUSTMART_CONFIG`/server); Postgres creds default to the docker-compose values (override via `JUSTMART_DB_{HOST,PORT,USER,PASSWORD,NAME,SSLMODE}`).
- **Postgres isolation = per-test schema**: `servicetest.NewDB` (under the env var) CREATEs a unique schema, opens the pool with `options='-c search_path=<schema>,public'` (the `,public` keeps the `citext` extension — created once in public — resolvable), migrates into it, and `DROP SCHEMA … CASCADE`s on `t.Cleanup`. Tests still assume a fresh/empty DB — that holds, since each schema is migration-seeded and otherwise empty. The Makefile bounds concurrency (`-p 1 -parallel 4`) so the per-test schema churn stays under PG's default lock table.
...
- **Shared fixtures = [internal/service/servicetest](backend/internal/service/servicetest/servicetest.go)** (imports only `auth`/`config`/`db`/`dbmigrate`/`model`/`service/user` → no import cycle from any domain test):
  - `servicetest.New(t) → (*gorm.DB, *config.Config)` — fresh throwaway DB per test (SQLite temp file by default; isolated PG schema under `JUSTMART_TEST_DB_DRIVER=postgres`) migrated via `dbmigrate.Run`. `NewDB(t,cfg)` / `NewConfig(t)` are the split forms; the engine branch is internal so test files never change.
